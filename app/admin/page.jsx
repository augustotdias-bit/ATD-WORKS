  ...
  if(!unlocked)return <main className="site admin-page">
    ...
    <section className="login-shell">
      <form className="login-card" onSubmit={login}>
        <p className="eyebrow">Private administration</p><h1>Admin</h1><p>Enter the private password to manage ATD WORKS.</p>
        <label><span>Password</span><input autoFocus type="password" autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)}/></label>
        <button className="primary-button" disabled={checking||!password} type="submit">{checking?'Checking…':'Enter'}</button>
  ...
  return <main className="site admin-page">
    ...
    <section className="admin-list">
      ...
      {works.length?<div>{works.map((w,index)=><div className={`admin-row ${w.hidden?'is-hidden':''}`} key={w.id}>
        {editing===w.id?<div className="edit-grid"><input value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})}/><input value={draft.medium} onChange={e=>setDraft({...draft,medium:e.target.value})}/><input value={draft.dimensions} onChange={e=>setDraft({...draft,dimensions:e.target.value})}/><input value={draft.date} onChange={e=>setDraft({...draft,date:e.target.value})}/><select value={draft.availability} onChange={e=>setDraft({...draft,availability:e.target.value})}><option>Available</option><option>Sold</option><option>Not for sale</option></select></div>:<div><strong>{w.title}{w.hidden?' · Hidden':''}</strong><span>{w.medium} · {w.dimensions} · {w.date} · {w.availability}</span></div>}
