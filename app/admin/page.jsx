...
export default function Admin(){
  ...
  useEffect(()=>{const saved=sessionStorage.getItem('atd-admin-password')||'';setPassword(saved);refresh(saved)},[]);
  ...
  async function unlock(){setMessage('');try{await refresh(password);setMessage('Archive loaded.')}catch{setMessage('Could not load archive.')}}
  ...
  function startEdit(work){setEditing(work.id);setDraft({title:work.title,medium:work.medium,dimensions:work.dimensions,date:work.date,availability:work.availability})}
  async function saveEdit(id){
    try{await updateWork(id,draft,password);setEditing(null);setMessage('Artwork updated.');await refresh()}
    catch(err){setMessage(err.message==='Unauthorized'?'Incorrect admin password.':'Could not update this artwork.')}
  }
  ...
  async function move(index,direction){
    const other=index+direction;if(other<0||other>=works.length)return;
    const a=works[index],b=works[other];
    try{await updateWork(a.id,{order:b.order},password);await updateWork(b.id,{order:a.order},password);await refresh()}catch(err){setMessage(err.message==='Unauthorized'?'Incorrect admin password.':'Could not reorder artworks.')}
  }
  ...
  return <main className="site admin-page">
    ...
    <section className="admin-shell">
      <div className="admin-intro"><p className="eyebrow">Archive administration</p><h1>Add<br/>work</h1><p>Upload new work here. Below, you can edit catalogue information, change availability, hide works temporarily and control their display order.</p></div>
      <form className="work-form" onSubmit={submit}>
        <label><span>Admin password</span><div className="password-row"><input type="password" autoComplete="current-password" placeholder="Required for changes" value={password} onChange={e=>rememberPassword(e.target.value)}/><button type="button" onClick={unlock}>Load archive</button></div></label>
    ...
    <section className="admin-list">
      ...
      {works.length?<div>{works.map((w,index)=><div className={`admin-row ${w.hidden?'is-hidden':''}`} key={w.id}>
        {editing===w.id?<div className="edit-grid">
          <input value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})}/>
          <input value={draft.medium} onChange={e=>setDraft({...draft,medium:e.target.value})}/>
          ...
          <select value={draft.availability} onChange={e=>setDraft({...draft,availability:e.target.value})}><option>Available</option><option>Sold</option><option>Not for sale</option></select>
        </div>:<div><strong>{w.title}{w.hidden?' · Hidden':''}</strong><span>{w.medium} · {w.dimensions} · {w.date} · {w.availability}</span></div>}
        <div className="admin-actions">
          {editing===w.id?<><button onClick={()=>saveEdit(w.id)}>Save</button><button onClick={()=>setEditing(null)}>Cancel</button></>:<button onClick={()=>startEdit(w)}>Edit</button>}
          <button title="Move up" onClick={()=>move(index,-1)}>↑</button><button title="Move down" onClick={()=>move(index,1)}>↓</button>
          <button onClick={()=>toggleHidden(w)}>{w.hidden?'Show':'Hide'}</button><a href={`/work/${w.id}`}>View</a><button onClick={()=>remove(w.id)}>Delete</button>
        </div>
